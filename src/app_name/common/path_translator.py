...
class PathTranslator:
    ...
    def _parse_mappings(self, mappings_env: str) -> dict[str, dict[str, str]]:
        ...
        for pair in mapping_pairs:
            ...
            is_windows_source = is_path_windows(normalized_source)
            is_windows_destination = is_path_windows(normalized_destination)
    ...
    def _find_mapping(self, path_str: str, category: str = "cross-platform") -> str | None:
        ...
        Args:
            ...
            category (str, optional): category for mapping lookup. Defaults to cross-platform.
        ...
        # Direct match first
        if path_str in mappings:
            return mappings[path_str]

        # Check for prefix matches (longest match wins)
        matching_mappings = [(source, target) for source, target in mappings.items() if path_str.startswith(source + "/") or path_str == source]
        ...
        relative_part = path_str[len(best_source) :].lstrip("/")
        return f"{best_target}/{relative_part}" if relative_part else best_target

    def to_path(self, path: str | Path, mapping: bool, exists: bool, target: str = "auto") -> str:
        ...
        Args:
            ...
            exists (bool): whether to check path existence.
            target (str, optional): target system to convert input path to. Defaults to auto.

        Returns:
            str: translated path as string.

        Raises:
            FileNotFoundError: if file or directory is not found.
        ...
        path_system = "windows" if is_path_windows(path_str) else "linux"
        ...
        if path_system == target_system:
            if mapping:
                mapped_path = self._find_mapping(path_str, target_system)
                if mapped_path:
        ...
        # Ensure path exists, whether a file or a directory
        if exists and not Path(path_str).exists():
...
def is_path_windows(path_str: str) -> bool:
    """Determine if path appears to be a Windows path.

    Args:
        path_str (str): path string to analyze.
...
#               ------- System ------
# ---------------------------------------------------------------------------- #
def to_path(path: str | Path, mapping: bool = False, exists: bool = True) -> Path:
    ...
    Args:
        ...
        exists (bool, optional): whether to check path existence. Defaults to True.

    Returns:
        Path: Native system translated path.
    """
    return Path(translator().to_path(path, mapping=mapping, exists=exists))


def to_pure_path(path: str | Path, mapping: bool = False, exists: bool = True) -> PurePosixPath | PureWindowsPath:
    ...
    if target_system == "windows":
        return PureWindowsPath(translator().to_path(path, mapping=mapping, exists=exists))
    return PurePosixPath(translator().to_path(path, mapping=mapping, exists=exists))


# ---------------------------------------------------------------------------- #
#               ------- Linux ------
# ---------------------------------------------------------------------------- #
def to_posix(path: str | Path, mapping: bool = True, exists: bool = False) -> Path:
    """Translate a path to Posix format.

    Args:
        path (str | Path): path to translate.
        mapping (bool, optional): whether to apply mapping. Defaults to True.
        exists (bool, optional): whether to check path existence. Defaults to False.

    Returns:
        Path: Posix translated path.
    """
    return Path(translator().to_path(path, mapping=mapping, exists=exists, target="linux"))


def to_pure_posix(path: str | Path, mapping: bool = True, exists: bool = False) -> PurePosixPath:
    ...
    """
    return PurePosixPath(translator().to_path(path, mapping=mapping, exists=exists, target="linux"))


# ---------------------------------------------------------------------------- #
#               ------- Windows ------
# ---------------------------------------------------------------------------- #
def to_windows(path: str | Path, mapping: bool = True, exists: bool = False) -> Path:
    ...
    """
    return Path(translator().to_path(path, mapping=mapping, exists=exists, target="windows"))


def to_pure_windows(path: str | Path, mapping: bool = True, exists: bool = False) -> PureWindowsPath:
    ...
    """
    return PureWindowsPath(translator().to_path(path, mapping=mapping, exists=exists, target="windows"))


# ---------------------------------------------------------------------------- #
#               ------- URI ------
# ---------------------------------------------------------------------------- #
def to_uri(path: str | Path, mapping: bool = True, exists: bool = False, target: str = "auto") -> str:
    """Translate path to corresponding URI.
    ...
    Returns:
        str: URI-formatted path.
    """
    path_str = translator().to_path(path, mapping=mapping, exists=exists, target=target)

    operating_system = "linux" if system() in ("Linux", "Darwin") else "windows"
    target_system = operating_system if target == "auto" else target

    return Path(path_str).as_uri() if operating_system == target_system else f"file:///{quote(path_str, safe='/:@')}"
